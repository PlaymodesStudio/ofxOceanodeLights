...
void panTiltEditor::setup(){
    for(int i = 0; i < 32; i++){
        pans.emplace_back();
        addParameter(pans.back().set("Pan " + ofToString(i), .5, 0, 1));
        listeners.push(pans.back().newListener([this](float &f){
            computePans();
        }));
        
        tilts.emplace_back();
        addParameter(tilts.back().set("Tilt " + ofToString(i), .5, 0, 1));
        listeners.push(tilts.back().newListener([this](float &f){
            computeTilts();
        }));
    }
    addOutputParameter(panOutput.set("Pan Output", {.5}, {0}, {1}));
    addOutputParameter(tiltOutput.set("Tilt Output", {.5}, {0}, {1}));
